...
{#if $loggedIn}
	<div class="flex flex-col items-end justify-center">
		<button
			type="button"
			class="relative mr-2 flex items-center rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200"
			on:click={(e) => {
				e.stopPropagation();
				isOpen = !isOpen;
			}}
		>
			...
			<div
				class="absolute top-0 z-50 mt-10 max-w-sm rounded-lg border border-gray-200 bg-white p-6 shadow transition-opacity
